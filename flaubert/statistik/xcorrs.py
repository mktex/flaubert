...
# QUELLE: https://github.com/shakedzy/dython/blob/master/dython/_private.py
...
def conditional_entropy(x, y, nan_strategy=REPLACE, nan_replace_value=DEFAULT_REPLACE_VALUE):
    ...
    **Returns:** float
    ...
    entropy = 0.0
    for xy in list(xy_counter.keys()):
...
def theils_u(x, y, nan_strategy=REPLACE, nan_replace_value=DEFAULT_REPLACE_VALUE):
    ...
    **Returns:** float in the range of [0,1]
    ...
    p_x = list([n / total_occurrences for n in list(x_counter.values())])
    s_x = ss.entropy(p_x)
    if s_x == 0:
        return 1
    else:
...
def associations(dataset, nominal_columns=None, mark_columns=False, theil_u=False, plot=True, return_results=False,
...
def numerical_encoding(dataset, nominal_columns='all', drop_single_label=False, drop_fact_dict=True,
                       nan_strategy=REPLACE, nan_replace_value=DEFAULT_REPLACE_VALUE):
    """
...
def check_corrs(xdfVis, xcolTarget, xcolID, korrThreshold=0.7):
    # zeige die Korrelationen für alle Spalten und stelle zur Auswahl, was soll den bleiben?
    print("[x] Korrelationen (numerische Features):")
    print(xdfVis.corr())
    xdfVis = xdfVis.copy()
    xdfVisArchiv = xdfVis.copy()
    xcolsKategs = [xcolTarget, xcolID]
    ok = True
    xL = []
    xcorrDict = {}
    while ok:
        j = 0
        xcols_redux = xdfVis.columns.values
        ok = False
        while j < len(xcols_redux):
            if xcols_redux[j] not in xcolsKategs:
                xcorrList = statistik.get_corrlist(xdfVis, xcols_redux[j])
                xcols = xcorrList[[np.abs(x) > korrThreshold for x in xcorrList]].index.tolist()
                xcols = [x for x in xcols if x != xcols_redux[j]]
                if len(xcols) > 0:
                    xcorrDict[xcols_redux[j]] = xcols
                    xL.extend(xcols)
                    xdfVis = xdfVis[[x for x in xdfVis.columns if x not in xcols]]
                    xcols_redux = xdfVis.columns.values
                    ok = True
            j += 1

    print("[x] Gefundene Korrelationen ( abs(j) > 0.8):")
    pprint(xcorrDict)

    for xkey in list(xcorrDict.keys()):
        print("====================================================================================")
        pprint(xcorrDict[xkey])
        print()
        xdfVis = xdfVisArchiv.copy()
        n = len(xcorrDict[xkey])
        if n > 5:
            blocks = [[(5 * i), (5 * (i + 1))] for i in range(n // 5)] + [[5 * (n // 5), 5 * (n // 5) + n % 5]]
            for b in blocks:
                xtemp = xdfVis[[xcolTarget, xkey] + np.array(xcorrDict[xkey])[list(range(*b))].tolist()]
                xdiagramme.show_summary_gridplots(xtemp, xkateg=xcolTarget, xcolID=xcolID)
        else:
            xtemp = xdfVis[[xcolTarget, xkey] + xcorrDict[xkey]]
            xdiagramme.show_summary_gridplots(xtemp, xkateg=xcolTarget, xcolID=xcolID)
